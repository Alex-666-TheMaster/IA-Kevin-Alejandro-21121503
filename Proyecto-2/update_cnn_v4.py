...
for i, cell in enumerate(nb['cells']):
    if cell['cell_type'] == 'code':
        ...
        # Identify MobileNetV2 definition cell
        if "base_model = MobileNetV2" in source_code:
            cell['source'] = new_model_source
            model_updated = True
            print(f"Model definition updated (replaced MobileNetV2) in cell {i}")
            
        # Identify compile cell
        if "from tensorflow.keras.optimizers import Adam" in source_code and "sport_model.compile" in source_code:
