...
for i, cell in enumerate(nb['cells']):
    if cell['cell_type'] == 'code':
        ...
        #     print(f"Found Sequential in cell {i}")
        #     print(source_code)

        # Identify model definition cell - using looser matching
        if "sport_model = Sequential" in source_code and "input_shape=(64,64,3)" in source_code:
            cell['source'] = new_model_source
            model_updated = True
            print(f"Model definition updated in cell {i}")
        ...
        if "sport_model.compile" in source_code and "categorical_crossentropy" in source_code:
            ...
            print(f"Model compile step updated in cell {i}")
...
if model_updated and compile_updated:
    ...
    if not model_updated: 
        print("- Model definition cell not found.")
        # Debug: check why
        for i, cell in enumerate(nb['cells']):
             if "Sequential" in "".join(cell.get('source', [])):
                 print(f"DEBUG: Found 'Sequential' in cell {i}, Content: {cell['source']}")
