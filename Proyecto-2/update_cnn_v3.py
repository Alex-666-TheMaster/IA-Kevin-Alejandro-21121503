...
for i, cell in enumerate(nb['cells']):
    if cell['cell_type'] == 'code':
        ...
        # Identify model definition cell - permissive
        if "Conv2D(32" in source_code and "input_shape=(64,64,3)" in source_code:
            cell['source'] = new_model_source
            model_updated = True
            print(f"Model definition updated in cell {i}")
            
        # Identify compile cell
        if "sport_model.compile" in source_code and "categorical_crossentropy" in source_code and "SGD" in source_code:
